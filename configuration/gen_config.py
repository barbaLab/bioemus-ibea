...
def gen_config(config_name:str, netw_conf_params:NetwConfParams, save_path:str="./"):
    ...
    swconfig_builder.parameters["bin_fmt_save_spikes"]         = True
    swconfig_builder.parameters["bin_fmt_save_vmem"]           = False
    swconfig_builder.parameters["bin_fmt_send_spikes"]         = True
    swconfig_builder.parameters["bin_fmt_send_vmem"]           = False
    ...
    # Ratbrain modeling #################################################################
        ...
        MAX_PCON     = netw_conf_params.org_pcon
        INH_RATIO   = netw_conf_params.org_inh_ratio
        EXC_RATIO   = netw_conf_params.org_exc_ratio
        ...
        ## (2) - Generate neurons
        org.genNeurons(inh_ratio=INH_RATIO)                                               # <EDIT>

        ## (3) - Generate synaptic connections
        for src in range(NB_ORGANOID):
            for dest in range(NB_ORGANOID):
                if src == dest: # within networks
                    org.genSynCon(rule="smallworld",     org_src=src, org_dest=dest, max_pcon=MAX_PCON, exc_ratio = EXC_RATIO) # <EDIT>
                else: # between networks
                    org.genSynCon(rule="smallworld",     org_src=src, org_dest=dest, max_pcon=MAX_PCON) # <EDIT>
        ...
        # Print (to plot the positions or synapses just uncomment one line)
        ...
        for dest in range(NB_NEURONS):
            for src in range(NB_NEURONS):
                ...
                if tsyn_i == "destexhe_none":
                    ...
                    wampa = netw_conf_params.org_wampa
                    wsyn_row.append(wampa*wsyn_i) # <EDITABLE>
                elif tsyn_i == "destexhe_gabaa":
                    wsyn_row.append(wsyn_i)
                elif tsyn_i == "destexhe_nmda":
                    wnmda = netw_conf_params.org_wnmda/2 ##### divided by two for a small bug in the NMDA weight
                    # print(wnmda)
    ...
    # Write file (to write text file with xy positions, neuron type, synapse type with all neurons)
    ...
    # Neuron types
    for n in tnrn:
        hhp = Hhparam().getParameters(n, dt, muRat = netw_conf_params.org_inj_noise)
