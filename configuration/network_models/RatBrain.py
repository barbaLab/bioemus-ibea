...
class RatBrain:
    ...
    def genSynCon(self, rule, org_dest, org_src, max_pcon, exc_ratio):
        ...
        if   rule == "single":
            ...
            self.tsyn = self.__ruleSynConSmallWorld(self.tsyn, self.nlist[org_dest], self.nlist[org_src], self.tnrn_all, self.x_all, self.y_all, self.org_diam[org_src], max_pcon, exc_ratio)
    ...
    # def __ruleSynConSmallWorld(self, tsyn, ndest, nsrc, tnrn_all, x_all, y_all, org_diam, pcon, exc_ratio):
    ...
    #                     elif tnrn_all[src] == NRN_EXC:
    #                         # print(inh_perc)
    #                         if (np.random.rand() < exc_ratio): # <EDIT>
    ...
    def __ruleSynConSmallWorld(self, tsyn, ndest, nsrc, tnrn_all, x_all, y_all, org_diam, pcon, exc_ratio):
        ## vectorized version
        ndest_tmp = np.array(ndest)
        nsrc_tmp = np.array(nsrc)
        x_all_tmp = np.array(x_all)
        y_all_tmp = np.array(y_all)

        # Precompute neuron positions as arrays
        x_dest, y_dest = x_all_tmp[ndest_tmp], y_all_tmp[ndest_tmp]
        x_src, y_src = x_all_tmp[nsrc_tmp], y_all_tmp[nsrc_tmp]

        # Create meshgrid of neuron positions for vectorized distance calculation
        x_diff = x_dest[:, np.newaxis] - x_src  # Difference in x-coordinates (dest x src)
        y_diff = y_dest[:, np.newaxis] - y_src  # Difference in y-coordinates (dest x src)

        # Vectorized distance calculation (dest x src matrix)
        distances = np.sqrt(x_diff**2 + y_diff**2)

        # Vectorized probability calculation (closer neurons have higher probability)
        probabilities = pcon * (1 - (distances**(13/10)) / org_diam)

        # Vectorized random probability matrix
        random_vals = np.random.rand(len(ndest), len(nsrc))

        # Vectorized connection decision based on probability
        connection_mask = (random_vals < probabilities)

        # Loop over valid connections and assign synapse types
        for i, dest in enumerate(ndest):
            for j, src in enumerate(nsrc):
                if dest != src and connection_mask[i, j]:  # Only valid connections
                    if tnrn_all[src] == NRN_INH:
                        tsyn[dest][src] = SYN_INH
                    elif tnrn_all[src] == NRN_EXC:
                        # Determine if the synapse is NMDA or AMPA
                        if np.random.rand() < exc_ratio:
                            tsyn[dest][src] = SYN_EXC_NMDA
                        else:
                            tsyn[dest][src] = SYN_EXC_AMPA
    ...
    def __genGroupSynWeights(self, ndest, nsrc, wsyn, w):
        ...
        #     for src in nsrc:
        #         wsyn[dest][src] = w
        wsyn = np.full((len(ndest), len(nsrc)), w)
        return wsyn
    ...
    def getSynTypes(self):
        ...
        #     tsyn_row_str = [syncode[self.tsyn[dest][src]] for src in range(self.nb_nrn)]
        #     tsyn_str.append(tsyn_row_str)
        tsyn_str = [[syncode[val] for val in row] for row in self.tsyn]
