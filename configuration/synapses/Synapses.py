...
class Destexhe:
    ...
    def __init__(self) -> None:
        ...
        NB_PSYN  = len(self.PID)
        self.psyn = [0.0]*NB_PSYN
        self.psyn[self.PID["AMPA_K1"]]    = 1.1e6 *1e-6   # [1/(mM.ms)]
        self.psyn[self.PID["AMPA_K2"]]    = 190   *1e-3   # [1/ms]
        self.psyn[self.PID["AMPA_Gsyn"]]  = 0.35          # 0.35 to 1.0 [nS]
        self.psyn[self.PID["AMPA_Esyn"]]  = 0.0           # [mV]
        self.psyn[self.PID["NMDA_K1"]]    = 7.2e4 *1e-6   # [1/(mM.ms)]
        self.psyn[self.PID["NMDA_K2"]]    = 6.6   *1e-3   # [1/sec]
        self.psyn[self.PID["NMDA_Gsyn"]]  = 0.30          # 0.01 to 0.6 [nS]
        self.psyn[self.PID["NMDA_Esyn"]]  = 0.0           # [mV]
        self.psyn[self.PID["GABAa_K1"]]   = 5e6 *1e-6     # [1/(mM.ms)]
        self.psyn[self.PID["GABAa_K2"]]   = 180 *1e-3     # [1/sec]
        self.psyn[self.PID["GABAa_Gsyn"]] = 0.25          # 0.25 to 1.2 [nS]
        self.psyn[self.PID["GABAa_Esyn"]] = -80.0         # [mV]
        self.psyn[self.PID["GABAb_K1"]]   = 9e4   * 1e-6  # [1/(mM.ms)]
        self.psyn[self.PID["GABAb_K2"]]   = 1.2   * 1e-3  # [1/ms]
        self.psyn[self.PID["GABAb_K3"]]   = 180   * 1e-3  # [1/ms]
        self.psyn[self.PID["GABAb_K4"]]   = 34    * 1e-3  # [1/ms]
        self.psyn[self.PID["GABAb_Gsyn"]] = 1.00          # 1.0 [nS]
        self.psyn[self.PID["GABAb_Esyn"]] = -95           # [mV]
        self.psyn_GABAb_Kd           = 100           #
        self.syn_GABAb_n            = 4             # nb binding sites
