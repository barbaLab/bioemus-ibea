...
class HwConfigFile :
    ...
    def write(self, fpath):
        ...
        with open(fpath, "w") as f:
            ...
            hh_param_lines = [
                f"HHparam_N{nrn}{KEY_SEP}{VAL_SEP.join(map(self.__formatFloat, self.HH_param[nrn]))}"
                for nrn in range(self.nb_nrn)
            ]
            f.write('\n'.join(hh_param_lines) + '\n')
            ...
            synaptic_config_lines = [
                f"N{nrn}{KEY_SEP}{VAL_SEP.join(f'{tsyn}${wsyn}' for tsyn, wsyn in zip(self.tsyn[nrn], self.wsyn[nrn]))}"
                for nrn in range(self.nb_nrn)
            ]  
            f.write('\n'.join(synaptic_config_lines) + '\n')      
