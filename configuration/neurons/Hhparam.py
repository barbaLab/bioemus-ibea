...
class Hhparam:
    ...
    def getParameters(self, nrn_type:str, dt, muRat = 0.0254):
        """Get HH parameters for a given neuron type

        :param str nrn_type: type of neuron ("FS","RS",...)
        :param dt float: time step in ms
        :param muRat: injected noise only for the RSrat neuron model
        ...
        if nt[0] == "FS":
            ...
            hhparam[self.PID["E_Leak"]]      = -70.0         # (mV)
            if not("nonoise" in nt):
                hhparam[self.PID["mu"]]      = muRat         # 0.05
            ...
            hhparam[self.PID["G_Na"]]        = 50e-3         # (S/cm²)
            hhparam[self.PID["G_Kd"]]        =  5e-3         # (S/cm²)
            hhparam[self.PID["G_M"]]         = 1.1e-4         # (S/cm²)
            hhparam[self.PID["G_L"]]         = 0.0           # (S/cm²)
            hhparam[self.PID["G_T"]]         = 0.0           # (S/cm²)
            hhparam[self.PID["G_Leak"]]      = 0.1e-3        # (S/cm²)
            hhparam[self.PID["E_Na"]]        = +50.0         # (mV)
            ...
            hhparam[self.PID["E_Leak"]]      = -70.0         # (mV)
            if not("nonoise" in nt):
                hhparam[self.PID["mu"]]      = muRat         # 0.1  <EDIT> injected noise
                hhparam[self.PID["theta"]]   = 6.2           # 8.0  <EDIT> injected noise
                hhparam[self.PID["sigma"]]   = 0.06          # 0.12 <EDIT> injected noise
            if not("nostim" in nt):
                hhparam[self.PID["i_stim"]]  = 0.03         # (mA/cm²)
            hhparam[self.PID["v_init"]]
