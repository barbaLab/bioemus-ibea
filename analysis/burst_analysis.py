...
def mean_bursting_rate(events, rec_duration_ms) : 
    mean        = []
    spikecounts = []
    time        = rec_duration_ms # in [ms]
    '''calculate the mean bursting rate of neuron signals'''

    for neuron in events : 
        spikecounts.append(len(neuron))
        mean.append(len(neuron)/(time/1e3)*60) # in [burst/min]

    return mean, spikecounts

def get_IBI(raster_list, events_list, rec_duration_ms) : 
    ...
    for events in events_list : 
        ...
        IBI_total = []
        MBR_total  = []
        MBR_total.append(mean_bursting_rate(burst_total, rec_duration_ms)[0])
    ...
    time_burst = np.logspace(np.log10(min([min(i) for i in IBI_file])),np.log10(max([max(i) for i in IBI_file])), 60)
    IBI_counts, bin_edges= np.histogram(IBI_total, bins=time_burst, density=False)
...
def get_burst_length(raster_list, events_list) : 
    ...
    for events in events_list : 
        ...
        for k in range(len(start)) :
            ...
            if len(start[k]) > 0 : # Takes only the neurons with at least one burst
                ...
                length_burst_total.append([np.nan])
        
        length_burst_total = np.concatenate(length_burst_total, axis=0)
        length_burst_file.append(length_burst_total)                      
    
    time_burst = np.linspace(0,max([max(i) for i in length_burst_file]), 60)
    length_burst_counts, bin_edges= np.histogram(length_burst_total, bins=time_burst, density=False)
    ...
    return length_burst_counts

def burst_analysis(raster_list, tstamp_list, rec_duration_s):
    MBR_total, IBI_global = get_IBI(raster_list, tstamp_list, rec_duration_s*1e3)
    length_burst_global = get_burst_length(raster_list, tstamp_list)
    return MBR_total, IBI_global, length_burst_global
