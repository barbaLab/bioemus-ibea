        ...
        for (int p = 0; p < _nb_hhparam; p++) {
            try {
                if (p == HHP_ID_NOISE_OFFS) {
                    ...
                    uint32_t hhp_val_int;
                    memcpy(&hhp_val_int, &hhp_val_fp, sizeof(hhp_val_int));
                ...
                cerr << "Error: out_of_range exception caught. Value: " << HHp_Ni[p] << " at parameter index " << p << " in neuron " << waddr << endl;
                cerr << "Exception message: " << e.what() << endl;
                return EXIT_FAILURE;  // or handle it as per your requirement
            }
            catch (const std::invalid_argument& e) {
                cerr << "Error: invalid_argument exception caught. Value: " << HHp_Ni[p] << " at parameter index " << p << " in neuron " << waddr << endl;
                cerr << "Exception message: " << e.what() << endl;
                return EXIT_FAILURE;  // or handle it as per your requirement
            }
        }
    ...
    for (int i = 0; i < _nb_ionrate; i++) {
        for (int waddr = 0; waddr < _depth_ionrate; waddr++) {
            ...
            try {
                ...
                cerr << "Error: out_of_range exception caught. Value: " << mh_rates[0] << ", " << mh_rates[1] 
                     << ", " << mh_rates[2] << ", " << mh_rates[3] << " at waddr " << waddr << " in ionrate " << i << endl;
                cerr << "Exception message: " << e.what() << endl;
                return EXIT_FAILURE;  // Return or handle it as needed
            }
            catch (const std::invalid_argument& e) {
                cerr << "Error: invalid_argument exception caught. Value: " << mh_rates[0] << ", " << mh_rates[1] 
                     << ", " << mh_rates[2] << ", " << mh_rates[3] << " at waddr " << waddr << " in ionrate " << i << endl;
                cerr << "Exception message: " << e.what() << endl;
                return EXIT_FAILURE;  // Return or handle it as needed
            }
...
int HwCfg::applySynrates() {
    ...
    for (int waddr = 0; waddr < _depth_synrate; waddr++) {
        ...
        try {
            // Fill AXI registers with error handling for stof
            float bv = stof(rates[0]);
            float tv = stof(rates[1]);
            float sn_gabab = stof(rates[2]);

            _axilite->write(bv, REGW_SYNRATE_BV, SFI_BRATE_SYN_INT, SFI_BRATE_SYN_DEC); // Bv
            _axilite->write(tv, REGW_SYNRATE_TV, SFI_TRATE_SYN_INT, SFI_TRATE_SYN_DEC); // Tv
            _axilite->write(sn_gabab, REGW_SN_GABAB, SFI_SN_GABAB_OUT_INT, SFI_SN_GABAB_OUT_DEC); // sn_gabab
        }
        catch (const std::out_of_range& e) {
            cerr << "Error: out_of_range exception caught for rates: " 
            ...
            cerr << "Error: invalid_argument exception caught for rates: " 
                 << rates[0] << ", " << rates[1] << ", " << rates[2] 
                 << " at waddr " << waddr << endl;
            cerr << "Exception message: " << e.what() << endl;
            return EXIT_FAILURE;
...
int HwCfg::applySynParams() {
    ...
    for (uint8_t wsel = 0; wsel < NB_PSYN; wsel++) {
        try {
            ...
            if (wsel == PSYN_ID_AMPA_K1 || wsel == PSYN_ID_NMDA_K1 || 
                wsel == PSYN_ID_GABAA_K1 || wsel == PSYN_ID_GABAB_K1) {
                fp_int = SFI_K1_SYN_INT;
                fp_dec = SFI_K1_SYN_DEC;
            }
            else if (wsel == PSYN_ID_AMPA_K2 || wsel == PSYN_ID_NMDA_K2 || 
                     wsel == PSYN_ID_GABAA_K2 || wsel == PSYN_ID_GABAB_K2) {
            ...
            else if (wsel == PSYN_ID_AMPA_ESYN || wsel == PSYN_ID_NMDA_ESYN || 
                     wsel == PSYN_ID_GABAA_ESYN || wsel == PSYN_ID_GABAB_ESYN) {
                fp_int = SFI_V_TRUNC_INT;
                fp_dec = SFI_V_TRUNC_DEC;
            }
            else if (wsel == PSYN_ID_AMPA_GSYN || wsel == PSYN_ID_NMDA_GSYN || 
                     wsel == PSYN_ID_GABAA_GSYN || wsel == PSYN_ID_GABAB_GSYN) {
                fp_int = SFI_GSYN_INT;
                fp_dec = SFI_GSYN_DEC;
            }
            else {
            ...
            reg_val = (((uint32_t)(wsel) & ((1 << PSYN_WIDTH_WSEL) - 1)) << PSYN_WIDTH_WDATA) | 
                       (float2sfi(wdata, fp_int, fp_dec) & ((1 << PSYN_WIDTH_WDATA) - 1));
            ...
            cerr << "Error: out_of_range exception caught for psyn[" << static_cast<int>(wsel) 
                 << "] = " << psyn[wsel] << " at wsel " << static_cast<int>(wsel) << endl;
            cerr << "Exception message: " << e.what() << endl;
            return EXIT_FAILURE;
        }
        catch (const std::invalid_argument& e) {
            cerr << "Error: invalid_argument exception caught for psyn[" << static_cast<int>(wsel) 
                 << "] = " << psyn[wsel] << " at wsel " << static_cast<int>(wsel) << endl;
            cerr << "Exception message: " << e.what() << endl;
            return EXIT_FAILURE;
        }
...
int HwCfg::applySynConf() {
    ...
    for (int dest = 0; dest < _nb_nrn; dest++) {
        ...
        for (int src = 0; src < _nb_nrn; src++) {
            ...
            try {
                float wsyn_value = stof(str_wsyn_i);
                twsyn[cnt_col_offs * 3 + cnt_col + (offs_line + offs_rest) * (NB_RAM_TWSYN * RAMWIDTH_TWSYN / 16) + offs_depth * NB_DSP_WMB_SYNAPSES * (_nb_nrn - 1)] = 
                    ((uint16_t)(tsyn_i) << WEIGHT_SIZE) | 
                    (float2sfi(wsyn_value, SFI_WSYN_INT, SFI_WSYN_DEC) & ((1 << WEIGHT_SIZE) - 1));
            } catch (const std::out_of_range& e) {
                cerr << "Error: out_of_range exception caught for str_wsyn_i = " << str_wsyn_i 
                     << " at src " << src << ", dest " << dest << endl;
                cerr << "Exception message: " << e.what() << endl;
                return EXIT_FAILURE;
            } catch (const std::invalid_argument& e) {
                cerr << "Error: invalid_argument exception caught for str_wsyn_i = " << str_wsyn_i 
                     << " at src " << src << ", dest " << dest << endl;
                cerr << "Exception message: " << e.what() << endl;
                return EXIT_FAILURE;
            }
    ...
    for (int waddr = 0; waddr < RAMDEPTH_TWSYN; waddr++) {
        for (int ram_i = 0; ram_i < NB_RAM_TWSYN; ram_i++) {
            ...
            _axilite->write(0, REGW_WEN_TWSYN_LSB);
            _axilite->write(0, REGW_WEN_TWSYN_MSB);
        }
    }
